...
setwd('~/experiments/sdm_keteleeria_huangwy88')
load('./wc/RData/ensemble_model.RData')

maxent_rc <- bm_PlotResponseCurves(bm.out = model_for_projection,
                                   models.chosen = get_built_models(model_for_projection,
                                                                    algo = 'MAXNET'))
write.csv(maxent_rc$tab, './wc/Evaluation/maxent_rc.csv')
...
# 获取"RUN"开头的列名
run_column_names <- grep("^keteleeria_allData_RUN\\d+_MAXNET$", names(maxent_rc_cleaned), value = TRUE)
