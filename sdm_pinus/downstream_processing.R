...
resp_curve <- bm_PlotResponseCurves(model_for_projection,
                                    get_built_models(model_for_projection, algo = 'GBM'))
resp_curve_data <- as.data.frame(resp_curve$tab) %>%
  ...
  mutate(avg_pred_val = rowMeans(select(., starts_with("pinus_allData_RUN"))),
         std_dev_pred_val = rowSds(as.matrix(select(., starts_with("pinus_allData_RUN"))))) %>%
  as.data.frame()
